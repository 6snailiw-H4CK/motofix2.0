...
// start scanning from the first `return (` occurrence to avoid matching TS generics
const returnIndex = lines.findIndex((l) => l.includes('return ('));
const scanStart = returnIndex >= 0 ? returnIndex : 0;
  ...
  while((m=tagRegex.exec(line))){
    ...
    if(closing){
      ...
      if(!top){
        ...
        console.log('Current stack (top first):', tags.map(t => `${t.tag}@L${t.line}:${t.col}`).reverse());
        process.exit(0);
      }
      if(top.tag !== tag){
        console.log(`Line ${i+1}, col ${col}: closing </${tag}> but top is <${top.tag}> opened at L${top.line}:${top.col}`);
        console.log('Current stack (top first):', [top, ...tags].map(t => `${t.tag}@L${t.line}:${t.col}`));
        process.exit(0);
      }
    } else {
